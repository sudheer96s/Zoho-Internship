...
def home():
    ...
    return render_template('index.html', buses=[], visitor_count=visitor_count, selected_date=date)
...
def book():
    ...
    with seat_locks[lock_key]:
        ...
        try:
            # Check if seat is available or in waiting state
            cursor.execute("""
            ...
            # Function to release the seat if not confirmed
            def release_seat():
                ...
                with seat_locks[lock_key]:
                    ...
                    """, (bus_id, seat_number, date))
                    seat_status = cursor.fetchone()
                    if seat_status and seat_status['status'] == 2:  # Still in waiting state
                        cursor.execute("""
                            UPDATE bus_seat_availability 
                            SET status = 0 
                            WHERE bus_id = %s AND seat_number = %s AND availability_date = %s
            ...
            # Start threads for release and booking confirmation
...
def confirm_booking():
    ...
    try:
        ...
        if seat_status[0] == 1:
            return "Seat is already booked.", 400
        elif seat_status[0] == 0 or 2:
            # Seat is in waiting state, proceed to confirm booking
            cursor.execute("""
                UPDATE bus_seat_availability
                SET status = 1
                WHERE bus_id = %s AND seat_number = %s AND availability_date = %s
...
# Function to simulate multiple client bookings using threading
...
def simulate_bookings():
    ...
    threads = []
    for i in range(15):  # Adjust the number of threads to simulate more clients
